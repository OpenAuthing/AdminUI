...
const NestedNavMenu = ({
    ...
    return (
        <div className="cursor-pointer">
            <div className="group flex items-center px-2 py-2.5 rounded hover:bg-primary-50 transition-colors duration-300"
...
const getIcon = (icon?: MenuIconType) => {
    switch (icon) {
        ...
        case 'Key':
            return (<Key className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        case 'Network':
            return (<Network className="size-5 stroke-gray-500 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        case 'ShieldCheckIcon':
            return (<ShieldCheckIcon className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        case 'MonitorSmartphone':
            return (<MonitorSmartphone className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        case "Fingerprint":
            return (<Fingerprint className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        case 'Palette':
            return (<Palette className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        case 'Settings':
            return (<Settings className="w-5 h-5 stroke-gray-600 group-aria-selected:stroke-blue-600 group-hover:stroke-blue-600" />)
        default:
            return <></>
    }
}
    ...
    useEffect(() => {
        ...
        for (const menu of menuRoutes) {
            if (!menu.parentId) {
                ...
                const parent: NavMenuItemType = menuRoutes.find(item => item.id === menu.parentId)
                if (parent) {
                    if (!parent.items) {
                        parent.items = []
                    }
